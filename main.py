...
@app.post("/users_Login", tags=["User"])
...
def update_campaign(campaign_id: UUID, payload: schemas.CampaignUpdate, db:Session= Depends(get_db), current_user: schemas.TokenData = Depends(get_current_user)):
...
def delete_campaign(campaign_id:UUID, current_user: schemas.TokenData = Depends(get_current_user), db: Session = Depends(get_db)):
